...
1. 大量 indexed 规则的语义与**仓库中已存在的 schedulex 子命令**高度匹配（例如所有 `RULE-OBJECT-*` / `RULE-ID-*` / `RULE-MODE-*` 都在描述 `schedulex goal-runtime` 已经在校验的对象模型契约）。
...
- **Goal 对象模型类**（OBJECT/ID/CONTROL/SSOT/ORPHAN/CONFLICT/MODE/MODE-GATE/CLASS/PRIORITY/ORDER/MILESTONE）→ `schedulex goal-runtime`
...
这些需要**新建 enforcer**（如 `schedulex dashboard-check` / `schedulex retro-check` / `schedulex self-improving-check`），属于 Batch 2+。
...
- **绑定语义漂移**：`schedulex goal-runtime` 实际检查的内容能否完全覆盖 12 条对象模型规则？Mitigation：本 PR 不修改任何工具行为，只是登记口对齐；未来 governance-fixture-test 可以为每条规则提供违规样例，那时若发现 enforcer 漏检，再调整 status。
- **后续 enforcer 改名**：若 `schedulex goal-runtime` 被拆分/改名，本批 12 条规则会一起失效。Mitigation：command-registry gate 已要求子命令稳定，且 registry.yaml 是机器生成，可一键重渲染。
