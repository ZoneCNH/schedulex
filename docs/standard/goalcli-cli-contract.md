...
- 直接运行 `schedulex <command> --json` 是只读检查，不会写 evidence。
...
`schedulex debt` supports `--section`, `--mode`, `--min-score`, and `--output json|markdown`. Aliases `architecture`, `domain`, `docs-drift`, `dependency-debt`, `testing-debt`, `implementation-debt`, and `security-debt` select the corresponding debt sections.
