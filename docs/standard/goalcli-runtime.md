# schedulex v1.0.0 runtime standard

Status: v1.0.0 release slice 的 normative authority。

## Authority

- `cmd/schedulex` 是 schedulex v1.0.0 commands 的唯一机器执行面。
- Harness Runtime 是 routing、policy 和 evidence interpretation 的 control plane。
- schedulex v1.0.0 不引入第二套并列执行面。
- `.agent/evidence/ledger.jsonl` 是 source evidence ledger。
- `release/evidence/schedulex/` 是 generated evidence pack 目录，不是 source ledger。

## Command surface

PR-4 command-backed slice 通过委托给 `cmd/schedulex` 的 Makefile targets 暴露 G12-G16 等价 gate：
...
每个命令都需要 `GOAL_ID` 或 `--goal-id`。同一 goal 的 fresh ledger-backed evidence 只有在 `goal-runtime-final` 调和全部 G12-G16 checks 后，才能报告 `release_status: complete`。

## Completion rule

只有同时满足以下条件，才能声明 schedulex v1.0.0 release complete：

- Harness policy 将 G12-G16 标记为 required。
- fresh command evidence 已记录到 `.agent/evidence/ledger.jsonl`。
- `release/evidence/schedulex/` 下的 generated packs 由 source ledger 派生。
