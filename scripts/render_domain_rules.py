...
输出:
  ...
  - .agent/rules/agent-runtime-rules.md    (AGENT/LEASE/HEARTBEAT/CMD-TXN/schedulex 等)
...
def main() -> int:
    ...
    # 收集源章节标题表（同时支持 # N. 与 ## N.）；
    # 只取首次出现，避免后续 v2.1 §314 中的步骤编号 (# 1. 初始化 schedulex 等) 覆盖真实章节。
