...
func (t onceTrigger) Next(after time.Time) (time.Time, bool) {
	if t.at.After(after) || t.at.Equal(after) { return t.at, true }
...
func (t intervalTrigger) Next(after time.Time) (time.Time, bool) {
	if t.every <= 0 { return time.Time{}, false }
	return after.Add(t.every), true
}

type dailyTrigger struct{ hour, min, sec int; loc *time.Location }
...
func (t dailyTrigger) Next(after time.Time) (time.Time, bool) {
	...
	if !n.After(local) { n = n.AddDate(0,0,1) }
	return n, true
}

type cronTrigger struct{ minuteStep int; minute *int; hour *int; loc *time.Location }
...
func Cron(expr string, loc *time.Location) (Trigger, error) {
	...
	if len(parts) != 5 { return nil, fmt.Errorf("schedulex: cron expects 5 fields") }
	for _, p := range parts[2:] { if p != "*" { return nil, fmt.Errorf("schedulex: only wildcard day/month/week fields supported") } }
...
func parseCronField(v string, min, max int) (int, *int, error) {
	if v == "*" { return 1, nil, nil }
	if strings.HasPrefix(v, "*/") {
		n, err := strconv.Atoi(strings.TrimPrefix(v, "*/")); if err != nil || n <= 0 || n > max+1 { return 0,nil,fmt.Errorf("schedulex: invalid cron step %q", v) }
		return n, nil, nil
	}
	n, err := strconv.Atoi(v); if err != nil || n < min || n > max { return 0,nil,fmt.Errorf("schedulex: invalid cron value %q", v) }
	return 0, &n, nil
}
...
func (t cronTrigger) Next(after time.Time) (time.Time, bool) {
	...
	for i:=0; i<366*24*60; i++ {
		...
		if t.hour != nil && c.Hour() != *t.hour { continue }
		if t.minute != nil { if c.Minute() != *t.minute { continue } } else if c.Minute()%t.minuteStep != 0 { continue }
		return c, true
	}
	return time.Time{}, false
}
