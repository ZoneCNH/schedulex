...
func (s *Scheduler) loop(job Job) {
	...
	var run int64
	for {
		next, ok := job.Trigger.Next(after)
		if !ok {
			return
		}
...
func (s *Scheduler) run(job Job, scheduled time.Time) {
	if job.Overlap != OverlapAllow {
		...
		if s.running[job.ID] {
			s.mu.Unlock()
			s.emit(Event{Type: EventSkipped, JobID: job.ID, At: s.clock.Now(), ScheduledAt: scheduled})
			return
		}
	...
	var lease Lease
	if job.Locker != nil {
		l, err := job.Locker.Lock(s.ctx, job.LockKey)
		if err != nil {
			s.emit(Event{Type: EventSkipped, JobID: job.ID, At: s.clock.Now(), ScheduledAt: scheduled, Error: err.Error()})
			return
		}
		lease = l
	...
	if err := job.Run(s.ctx); err != nil {
		s.emit(Event{Type: EventFailed, JobID: job.ID, At: s.clock.Now(), ScheduledAt: scheduled, Error: err.Error()})
		return
	}
